...
const WeeklyReset = () => {
    var date = new Date();
    var hoursMinSecs = Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate() + (10 - date.getUTCDay()) % 7 + 1, 0, 0, 0);
    var now = Date.now();
    ...
    if (hours === 0 && minutes === 0 && seconds === 0) {
        date = new Date();
        hoursMinSecs = Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate() + (7 - date.getUTCDay()) % 7 + 1, 0, 0, 0);
        now = Date.now();
    ...
    return (
        <div><span>{hours.toString().padStart(2, '0') + 'hrs ' + minutes.toString().padStart(2, '0') +
            'mins ' + seconds.toString().padStart(2, '0') + 'secs'}</span></div>
    );
}
